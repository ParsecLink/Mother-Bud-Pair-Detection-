...
Ajit's YeastPair project is developing an image-analysis workflow that detects and classifies budding-yeast biological units from multi-channel microscopy. The intended model output is one class and one bounding box per unit:
...
Questions still requiring Ajit's or the PI's decision are listed at the end of this document.
...
## 16. Questions Requiring Ajit's Input
